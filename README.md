A CLI tool for creating and reviewing flashcards. Currently a work in progress.

Ideas:
* Should probably have separate decks / subjects, and a way to pick which one you want. Maybe this can initialize separate databases. 
* Should probably have a way to mark a card as having different states, like correct / incorrect, etc.
* Need a study algorithm
* Should probably have a way for users to delete cards
* Workflow for now: pick a function to implement, start working on it in main, include functions that don't exist yet, and after
    outlining this pseudo-code, implement the functions in utils and/or db.
* Each time I do this, I should implement at least one unit test for the main function, plus at least one for each util/db function.